...
class StripedHyena(nn.Module):
    ...
    def forward(self, x, inference_params_dict=None, padding_mask=None, genome_idx=None, genome_chunk_idx=None):
        # print("x shape before embedding: ", x.shape, flush=True)
        L = x.shape[1]
        ...
        # print("x shape before final norm: ", x.shape, flush=True)
        # torch.save(x[:, -1, :], f"last_hidden_rep_{genome_idx}_{genome_chunk_idx}.pt")
        

        x = self.norm(x)
        x = self.unembed.unembed(x)
        return x, inference_params_dict_out, x[:, -1, :]
